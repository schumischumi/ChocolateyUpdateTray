...
if($noReturn){
    ...
    $icon = [System.Drawing.Icon]::ExtractAssociatedIcon("C:\ProgramData\chocolatey\choco.exe")


    
    # Add icon the systray
    $Main_Tool_Icon = New-Object System.Windows.Forms.NotifyIcon
    $Main_Tool_Icon.Text = "Chocolatey outdated error!"
    $Main_Tool_Icon.Icon = $icon
    $Main_Tool_Icon.Visible = $true

    # Add menu Chocolatey Info
    $Menu_INFO = New-Object System.Windows.Forms.MenuItem
    $Menu_INFO.Text = "Chocolatey outdated error!"
    ...
    $fix_path = $stdout_choco_raw.split('"')[2].trim()
    $Menu_FIX = New-Object System.Windows.Forms.MenuItem
    $Menu_FIX.Text = "Fix: remove $fix_path"
    $Menu_FIX.Add_Click({
        start-process powershell.exe -Verb RunAs -ArgumentList "remove-item $fix_path"
        $Main_Tool_Icon.Visible = $false
        #$window.Close()
        start-process powershell -ArgumentList "-windowstyle hidden  -ExecutionPolicy Bypass -File $PSCommandPath"
        #Stop-Process $pid
    })
    $Main_Tool_Icon.contextMenu.MenuItems.AddRange($Menu_FIX)
